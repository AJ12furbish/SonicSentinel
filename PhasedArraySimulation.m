...
%% 8-Channel of microphone capture parameters

% Two speech signals are used as audio of interest. A laughter audio segment
% is used as interference. The sampling frequency of the audio signals is
% 48 kHz. 

% azimuth: ? degrees, elevation: ? degrees
ang_femalespeech = [-30; 0]; 
...
%% Simulate 8-Channel 24-bit Capture with 8 bits of padding (256 bits/cycle)
...
for m = 1:NSampPerFrame:NTSample
    ...
    x1 = femaleSpeechFileReader();
    x2 = laughterFileReader();
    x3 = 2*maleSpeechFileReader();
    temp = collector([x1 x2 x3], ...
        [ang_femalespeech ang_laughter ang_malespeech]) + ...
...
% and writes them into a single multi-channel WAV file named 'output_simulated_audio.wav'. 
...
filename = 'output_simulated_audio.wav';
audiowrite(filename, sigArray, fs); 

disp(['8-channel audio saved to: ' filename]); 
